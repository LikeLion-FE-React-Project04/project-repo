...
function checkUiType(uiType) {
  switch (uiType) {
    case 'inquiry':
      return {
        title: '문의하기',
        ph: <PlaceholderInquiry />,
        collectionName: 'inquiryData',
      };
    case 'review':
      return {
        title: '후기 작성',
        ph: <PlaceholderReview />,
...
export function ProductDetailPopUp({ uiType = 'inquiry' }) {
  ...
  const textareaInputHandler = (e) => {
    ...
    if (uiType == 'inquiry') {
      ...
      if (
        textStateRef.current.textarea != '' &&
        textStateRef.current.title != ''
      ) {
        setIsReady(true);
      } else {
        setIsReady(false);
  ...
  const handleModalKeyEvent = (e) => {
    ...
    if (registrationBtnRef.current.disabled == true) {
      // 등록버튼이 disabled라면,
            ...
            <fieldset className={styles.inquiryContentWrapper}>
              ...
              >
                <textarea
                  ...
                  required
                  maxLength="5000"
                  onBlur={handlePlaceholderT}
                  onFocus={handlePlaceholder}
                ></textarea>
                {isActiveP ? ph : null}
                <p className={styles.inquiryCount} id="inquiryCount">
                  {inputCount}/5000
                </p>
              </div>
            </fieldset>
